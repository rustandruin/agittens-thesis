...
U = dct(eye(n)); % make singular spaces maximally incoherent
V = dct(eye(n));

indices = 0:(n-1);
pow = 1; % control the (power law) rate of decay
M = 100;
s = M*(1 - 1/n*indices).^pow;
fnorms = cumsum(s.^2);
S = diag(s);
A = U*diag(s)*V;
...
kmax = floor(n/(2*log(n))); % choose so 2*kmax*log(n) < 500
numpts = 30;
...
th = title('Relative residual spectral errors for $\mathbf{C}$');
set(th, 'interpreter', 'latex');
lh = legend('$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_2 / \|\mathbf{C} - \mathbf{C}_k\|_2$, Gaussian', ...
            '$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_2/ \|\mathbf{C} - \mathbf{C}_k\|_2$, SRHT', ...
            'Location', 'SouthWest');
%lh = legend('$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_2 / \|\mathbf{C} - \mathbf{C}_k\|_2$, Gaussian', ...
%            '$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_2/ \|\mathbf{C} - \mathbf{C}_k\|_2$, SRHT', ...
%            '$\|\mathbf{C} - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_2/\|\mathbf{C} - \mathbf{C}_k\|_2$, Gaussian', ...
%            '$\|\mathbf{C} - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_2/\|\mathbf{C} - \mathbf{C}_k\|_2$, SRHT', 'Location', 'SouthWest')
set(lh, 'interpreter', 'latex');

printcf('experimentC-residual-spectral.pdf', 16, 7, 6);
...
xlabel('k (target rank)');
yh = ylabel('relative error');
th = title('Relative residual Frobenius errors for $\mathbf{C}$');
set(th, 'interpreter', 'latex');
lh = legend('$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_F / \|\mathbf{C} - \mathbf{C}_k\|_F$, Gaussian', ...
            '$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_F / \|\mathbf{C} - \mathbf{C}_k\|_F$, SRHT', ...
            'Location', 'SouthWest');
% lh = legend('$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_F / \|\mathbf{C} - \mathbf{C}_k\|_F$, Gaussian', ...
%             '$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_F / \|\mathbf{C} - \mathbf{C}_k\|_F$, SRHT', ...
%             '$\|\mathbf{C} - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_F / \|\mathbf{C} - \mathbf{C}_k\|_F$, Gaussian', ...
%             '$\|\mathbf{C} - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_F / \|\mathbf{C} - \mathbf{C}_k\|_F$, SRHT', 'Location', 'SouthWest');
set(lh, 'interpreter', 'latex');

printcf('experimentC-residual-frobenius.pdf', 16, 7, 6);
...
xlabel('k (target rank)');
yh = ylabel('relative error');
th = title('Relative forward spectral errors for $\mathbf{C}$');
set(th, 'interpreter', 'latex');
lh = legend('$\|\mathbf{C}_k - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_2 / \|\mathbf{C}_k\|_2$, Gaussian', ...
            '$\|\mathbf{C}_k - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_2/ \|\mathbf{C}_k\|_2$, SRHT', ...
            '$\|\mathbf{C}_k - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_2/\|\mathbf{C}_k\|_2$, Gaussian', ...
            '$\|\mathbf{C}_k - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_2/\|\mathbf{C}_k\|_2$, SRHT', 'Location', 'SouthEast');
set(lh, 'interpreter', 'latex');

printcf('experimentC-forward-spectral.pdf', 16, 7, 6);
...
xlabel('k (target rank)');
yh = ylabel('relative error');
th = title('Relative forward Frobenius errors for $\mathbf{C}$');
set(th, 'interpreter', 'latex');
lh = legend('$\|\mathbf{C}_k - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_F / \|\mathbf{C}_k\|_F$, Gaussian', ...
            '$\|\mathbf{C}_k - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C} \|_F/ \|\mathbf{C}_k\|_F$, SRHT', ...
            '$\|\mathbf{C}_k - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_F/\|\mathbf{C}_k\|_F$, Gaussian', ...
            '$\|\mathbf{C}_k - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_F/\|\mathbf{C}_k\|_F$, SRHT', 'Location', 'NorthEast');
set(lh, 'interpreter', 'latex');

printcf('experimentC-forward-frobenius.pdf', 16, 7, 6);
...
th = title({'Number of samples needed to achieve $\|\mathbf{C} - \mathbf{L}\|_F \leq (1+\varepsilon) \|\mathbf{C} - \mathbf{C}_k\|_F$ with probability at least 1/2'});
set(th, 'interpreter', 'latex');
lh = legend('$\mathbf{L} = \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C}, \varepsilon = 1/2$', ...
            '$\mathbf{L} = \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C}, \varepsilon = 1/3$', ...
            '$\mathbf{L} = \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C}, \varepsilon = 1/6$', ...
            '$\mathbf{L} = \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C}), \varepsilon = 1/2$', ...
            '$\mathbf{L} = \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C}), \varepsilon = 1/3$', ...
            '$\mathbf{L} = \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C}), \varepsilon = 1/6$', ...
            'Location', 'SouthEast');
set(lh, 'interpreter', 'latex');
set(yh, 'interpreter', 'latex');

printcf('experimentC-empirically-necessary-r.pdf', 16, 12, 6);
...
th = title('Guaranteed vs observed spectral errors for $\mathbf{C}$');
set(th, 'interpreter', 'latex');
lh = legend('spectral error bound of Thm 4 / $\|\mathbf{C} - \mathbf{C_k}\|_2$', ...
            '$\|\mathbf{C} - \mathbf{P}_{\mathbf{C}\mathbf{S}} \mathbf{C}\|_2 / \|\mathbf{C} - \mathbf{C_k}\|_2$',...
            '$\|\mathbf{C} - \mathbf{\Pi}_{\mathbf{C}\mathbf{S}, k}^{\mathrm{F}}(\mathbf{C})\|_2 / \|\mathbf{C} - \mathbf{C}_k\|_2$');
set(lh, 'interpreter', 'latex');

printcf('experimentC-actual-versus-predicted-spectral-error.pdf', 16, 6, 6);

%% Save data so the visualization parts can be run without the data generation parts

save('experimentCdata.mat');